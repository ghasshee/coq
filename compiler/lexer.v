Set Warning "-notation-overridden, -parsing, -deprecated-implicit-arguments".
...
Fixpoint tokenize_helper (cls : chartype) (acc xs : list ascii) : list (list ascii) := 
    ...
    match xs with 
    | []        =>  tk 
    | x::xs'    =>  match cls, classifyChar x, x with 
        | _,_,"("       => tk ++ ["("] :: (tokenize_helper other [] xs') 
        | _,_,")"       => tk ++ [")"] :: (tokenize_helper other [] xs') 
        | _,white,_     => tk ++ (tokenize_helper alpha (x::acc) xs') 
...
Example tokenize_ex1 : tokenize "abc12==3 222*(3+(a+c))" %string 
    = ["abc"; "12"; "=="; "3"; "223"; "*"; "("; "3"; "+"; "("; "a"; "+"; "c"; ")"]%string.
Abort. 


Definition hoge := tokenize "1*2* 3". 
